...
%% The feature extraction is processed by the ALIZE module of the ELISA
%% Consortium (Magrin- Chagnolleau et al., 2001). First, this one sampled
%% the speech signal every 10ms on a 20ms sliding window. From this
%% window, 16 Linear Frequency Cepstral Coefficients (LFCC) are
%% extracted.  Then, the 16 first order deltas are computed. A
%% 33-dimensional cepstral vector is obtained by taking the 16 LFCCs, the
%% 16 first order deltas and the delta energy. Finally, normalisation is
%% performed such that the 33 features are centred on zero and scaled to
%% an unitary variance. Fur- thermore, cepstral vectors corresponding to
%% the silence are skipped by a Bi-Gaussian model of the speech energy.
